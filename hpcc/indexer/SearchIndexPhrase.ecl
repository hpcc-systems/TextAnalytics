...
*/
export SearchIndex(DATASET($.layouts.CommandStack) instack,
...
// since this part of the code does not do pre/ then it cannot have the AND NOT section
// (if the AND NOT section has pre/)
stack := instack(not_phrase = false);
...
haveachance := IF(COUNT(stack) > 1 AND COUNT(stack) < 20, $.Functions.FN_HasAChanceFilter(bmpi, stack), GROUP(SORTED(bmpi, source_id, col_item), source_id, col_item));
// haveachance := bmpi;
rawBitmapDataDS := DATASET(rawBitmapDataFileName, {$.Indexes.BitmapLayout, UNSIGNED8 fpos{virtual(fileposition) }}, THOR); 
...
layout_accum addMissingViaJoin(groupsr L, stack R) := TRANSFORM
  ...
  SELF.accumOffset1 := [];
END;
jsr1 := JOIN(groupsr, stack, LEFT.search_id = RIGHT.search_id, addMissingViaJoin(LEFT, RIGHT), LEFT OUTER, GROUPED);
...
// Do MatchPhrase if PRE or NOT
normresult3 := IF(COUNT(instack(opcode_num IN [3, 9, 19])) > 0  , $.Functions.FN_MatchPhrase(normresult2, instack, colWordPosIndex), normresult2);
normresult4 := CHOOSEN(normresult3, pageSize, startoffset);
