...
/**
 * @author uleeddx
 * Utility class used by multiple classes 
 *
 */
...
public class SOAPHelper {
	...
	private static Logger _logger = Logger.getLogger(SOAPHelper.class);


	public static String makeAuth(String hpccLoginId, String hpccLoginPassword) {
		return Base64.encodeBase64String((hpccLoginId+":"+hpccLoginPassword).getBytes());
	}
...
//			String eclStr=new String(eclInput.getBytes("ISO-8859-1"),"UTF-8");
	        		...
	        		&& result.item(0).getChildNodes().item(0).getNodeType()==3)))
	        {
		        ...
		        {
		        	sb.append(getNodeValue(result.item(i)));		        	
		        }
	        }
	...
	/**
	 * GetNodeValue
	 * @param node - the node whose value to return. 
	 * @return string of the node's content.
	 * @throws TransformerFactoryConfigurationError 
	 * @throws TransformerException 
	 */
	public static String getNodeValue(Node node ) throws TransformerFactoryConfigurationError, TransformerException {
		if (node == null ) {
			return "";
		}
    	...
    	// Look for a remove namespace
    	int strt;
		if ((strt=result.indexOf(" xmlns=")) >= 0) {
    		int end = result.indexOf("\" ", strt);
    		if (end > strt) {
    			result = result.substring(0,strt)+result.substring(end+1);
    		}
    	}
	...
	public static String processRequest(String theURL, String data,
		...
		return processRequest(theURL,data,auth,new HashMap<String,String>());
	}
	public static String processRequest(String theURL, String data,
			String auth,HashMap<String,String> headerParams) throws Exception {
...
//			connection.connect();
    ...
    static public String transformXml(String xml, String xslfile,ServletContext ctx) throws Exception
    {
    	  try {
    	        ...
    	        //path should begin with dir under the context root, and start with "/"
    	        		...
    	        		InputStream is= ctx.getResourceAsStream(xslfile);
    	    	...
    	    	_logger.error("error in Utility.TransformXML:" + e.toString());
    ...
    public static String getEspError(String edfsResult)
    {
		...
		{
			error=SOAPHelper.getNodeValue(edfsResult,"//Exception");
			if (error.equals("") && edfsResult.contains("<body"))
			{
				error=SOAPHelper.getNodeValueString(edfsResult,"body");
			}
			if (error.equals(""))
			{
				error=edfsResult;
